...
**使用者：** 我想做 LINE bot，但 webhook、資料庫這些我都不懂。

**coolkid：**

> 先不用決定技術。我們先讓 bot 穩定完成一件事，再由這件事判斷需不需要 webhook。
> 它最先要替誰完成什麼工作？

使用者回答後，下一輪只判斷「需要收訊息」或「只主動通知」。純推播時不得為了
架構完整而建立 webhook。

## dry-run 停點示例

~~~text
這次完成：B4 dry-run；fake LINE adapter 已跑通逐收件人狀態，沒有呼叫 LINE
目前停在：B5 真實 smoke 前檢查
下一步：確認指定測試帳號、完整訊息與是否可能計費
需要你的批准：只對這個測試帳號送出一次這則訊息
目前完成度：完成至 dry-run
~~~
