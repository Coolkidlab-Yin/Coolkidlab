...
| **知識圖卡 / 教學** | 自己的文章、講義、問答 | 程式生多張輪播 | 固定頻率 | 文章 slug 或網址 | 一張塞太多字,手機看不清 |
...
- 套件:`pip install pillow requests python-dotenv` 是共通的;
  另外**依型態加**:要用 LLM 才裝 `anthropic`、要吃 RSS 才裝 `feedparser`
- 一台「會一直開著」的機器 — 依第 -1 步選的頻率決定要多常跑。
  自己的電腦關機就停,長期跑要放常開的機器或雲端排程服務
  (**只有「有新素材才發」的型態可以接受偶爾漏跑**,固定頻率型不行)
...
**鑰匙 1|LLM API key**(約 5 分鐘)

**先判斷要不要這把。** 不用 LLM 的型態(作品集、語錄、商品上新這種
「素材與文案都是現成的」)整把跳過,直接看鑰匙 2。硬要接 LLM 只會多一份
成本與一個會壞的環節。
...
**多張輪播**(知識圖卡型的預設做法)是三段,不是兩段:

```
# 1. 每一張圖各建一個「輪播項目」容器,帶 is_carousel_item=true
POST https://graph.instagram.com/v25.0/<IG_USER_ID>/media
     ?image_url=<第 N 張的公開網址>&is_carousel_item=true&access_token=<token>
     → 各回傳一個容器 ID

# 2. 把所有項目容器組成一個父容器
POST https://graph.instagram.com/v25.0/<IG_USER_ID>/media
     ?media_type=CAROUSEL&children=<ID1>,<ID2>,<ID3>&caption=<文案>&access_token=<token>
     → 回傳父容器 ID

# 3. 發布父容器(跟單張一樣)
POST https://graph.instagram.com/v25.0/<IG_USER_ID>/media_publish
     ?creation_id=<父容器 ID>&access_token=<token>
```

**一則輪播最多 10 張**(圖片、影片或混合)。文案掛在**父容器**上,
不是掛在個別項目上 —— 這點寫錯的話文案會消失。

> 做影片 / Reels 的話 `media_type` 要換成對應的值,而且要走影片上傳流程,
> 跟上面的圖片流程不同。這份文件沒涵蓋影片,請當場查官方文件。
...
3. dry-run 跑完,`logs/preview/` 裡有圖,而且**在手機尺寸下看起來對**。
   **圖上有文字的型態**(新聞、知識圖卡、語錄)要特別確認中文有正常顯示
   —— 不是空白也不是方框;**純圖片的型態**(作品集、商品照)則是確認
   尺寸與裁切沒跑掉。草稿檔裡有一段像人寫的文案 → 產線通了
...
- **AI 產出要誠實標示**:文案主要由 AI 產出的話,帶一句說明,不假裝是
  純人工。做商業用途(商品、活動)尤其不要讓人以為是真人一則一則寫的。
- **secrets 不落地**:IG token、LLM API key、圖床 key 一律走環境變數
  或秘密管理工具,不硬編碼、不 commit。
- **先 dry-run 再真發**:自動發文最怕「發出去才發現寫爛或圖醜」,
  預覽機制不是選配。
- **圖片的權利要乾淨**:自動流程最容易在這裡出事 —— 抓來的圖、素材庫的
  圖、AI 生成的圖,授權各不相同。作品集、商品型用自己的圖沒問題;
  新聞型不要直接抓別人的新聞配圖。
...
| 商品上新 / 促銷 | 價格與庫存要跟系統一致(發出去才發現賣完最傷);廣告不實的風險自己扛 |
| 知識圖卡 / 教學 | 內容錯了會被當成專業背書,AI 寫的知識類內容發前要有人看過 |
| 作品 / 攝影 | 有人物的照片要有肖像同意 |
| 活動宣傳 | 時間地點價格一個字都不能錯,錯了要立刻改貼文 |
| 涉及金融、醫療、法律 | **不給個人化建議、不保證結果**。這類主題的自動化風險最高,先確認他知道 —— 原作者的新聞版本就是靠這條把自己框住:只轉述事件,不寫買賣結論 |
